class User::JobsController < ApplicationController
  ...
  before_filter :find_job_posted, :only => [:show, :edit, :update, :destroy]
   ...
   def create
     @job = @company.jobs.new(params[:job].merge({:company_name => @company.name, :address_street1 => @company.address_street1, :address_street2 => @company.address_street2, :post_code => @company.post_code, :state => @company.state, :country => @company.country}))
     if @job.save
